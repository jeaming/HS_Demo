...
void CalMac(UCHAR *initdata,UCHAR *pin,UCHAR PinLen,UCHAR *dessrc,UCHAR DesSrcLen,UCHAR *Mac)
{
	UCHAR InitData[200],DesSrc[200],Pin[20];
	int i,j;
	int data_a,data_b;

	memcpy(InitData,initdata,8);
	memcpy(Pin,pin,PinLen);
	memcpy(DesSrc,dessrc,DesSrcLen);
	data_a=DesSrcLen/8;
	data_b=DesSrcLen%8;
	memset(&InitData[4],0,4);

	for(i=0;i<data_a;i++)
	{
		for(j=0;j<8;j++)InitData[j]=InitData[j]^DesSrc[i*8+j];
		mwdes(&Pin[0],InitData,InitData,1);
	}
	DesSrc[i*8+data_b]=0x80;
	for(j=1;j<8-data_b;j++)DesSrc[i*8+data_b+j]=0;
	for(j=0;j<8;j++)InitData[j]=InitData[j]^DesSrc[i*8+j];
	mwdes(&Pin[0],InitData,InitData,1);
	if(PinLen==16)  //triple DES
	{
		mwdes(&Pin[8],InitData,InitData,0);
		mwdes(&Pin[0],InitData,InitData,1);
	}
	memmove(Mac,InitData,4);
}
